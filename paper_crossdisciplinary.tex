...
\begin{abstract}
\noindent
Fast radio bursts are millisecond radio flashes from point sources on the sky,
and a few
known sources repeat, producing thousands of flashes over months. In the public
archives of three such repeaters, the bursts, read in order of arrival, carry
organised sequential structure, and the statistics that describe it follow each
source across different telescopes. We apply two standard toolkits, exactly as
published, and report what each returns; every statistic is computed from burst
order and relative spectral shape alone, independent of the sources' distances.

\smallskip
\noindent
\textbf{1. The protocol.} The toolkit engineers rely on to recover unknown
transmission formats from captured traffic
finds the working parts of one in FRB~20201124A, and the same architecture in
the two further repeaters tested. Each
burst contributes one symbol, classified from the shape of its radio spectrum;
one dominant component shared by the spectra plays the carrier, with five
weaker sub-channels multiplexed beneath it, and the symbol sequence itself
shows a repeating 32-symbol frame, a
line code that holds each symbol until new data arrives, and dependencies that
run one way in time.

\smallskip
\noindent
\textbf{2. The stripping.} Read whole, machinery and all, the sequence does
not fit language, and should not: framing and carrier drown whatever they
deliver. So exactly the machinery of step 1 is stripped away, with no
reference to what it carries, nothing
more: discard the carrier, separate the channels, the operation any receiver
performs. What remains is whatever the transport layer was carrying, and it
can be tested on its own.

\smallskip
\noindent
\textbf{3. The language test.} The statistical tests relied on to interrogate
undeciphered scripts and to search whale song for language-like structure,
applied to
what remains, pass on every axis, symbol frequencies, vocabulary growth and
predictive depth, matching English or exceeding it: context
helps prediction 18 symbols back where English text run through the identical
pipeline, matched in length and alphabet, manages 2.3 symbols, and six
multi-burst motifs learned in early sessions recur in later ones. The
structure survives every control built to explain it away, including the
memoryless control that unmade earlier entropy-based claims for undeciphered
scripts, and a
surrogate built to repeat symbols exactly as often, and in runs exactly as
long, as the real stream. Every prediction that could be tested on the second
repeater, stated before its archive was analysed, was confirmed. And the third
repeater ties the structure to the source, not the instrument: the same
fingerprint tracks one and the same repeater through two different telescopes,
while
different sources through one telescope stay distinct.
...
explicitly. That is the correct division, and it is the one this paper keeps:
\emph{we detected language}, in the closing line above, is the same kind of claim
those journals printed, the measured kind, that the stream passes every test the
language-detection toolkit possesses, with the semantic question bracketed just as
explicitly (Section~\ref{sec:reallylanguage}). This paper does not merely adopt the
...
individual flash, is the object.} Distance is outside that object: sources
are conventionally placed at cosmological distances by dispersion modelling
and, for several repeaters, by host-galaxy association; the dispersion route
bounds distance only from above and can overshoot
substantially where host galaxies have been measured
\cite{kumarlinder2019,ocker2022}, and the foreground
components of its models are still being revised against in-situ
measurement. Nothing here rests on placement; every statistic below is
computed from arrival order and relative spectral shape, and is unchanged by
any rescaling of distance or absolute strength.
...
\subsection{The results in brief}
\label{sec:brief}

For readers who want the numbers before the narrative: the three parts of
the argument with their headline measurements. Every value below is derived,
controlled and cross-checked in the sections that follow, and printed by the
deterministic scripts of the availability section.
...
\paragraph{The protocol.} The stream is organised into repeating frames of 32 symbols,
...
\paragraph{The contents.} Applied directly to burst parameters the standard two-axis
...
\paragraph{The controls, in increasing severity.} Shuffling burst order while keeping every
...
Per-offset Shannon entropy read vertically across aligned messages is a standard
...
One further observation about the same direction, stated as statistics and nothing
more. A dependency span of 18 against matched English's 2.3 is not the signature of
something looser than language; it is the signature of something \emph{globally
constrained}, a system whose distant parts must stay mutually consistent for the
whole to work. The canonical terrestrial example is executable code. Under the
identical language-model pipeline, source code is markedly \emph{more} predictable
locally than English \cite{hindle2012}, and it carries the same class of long-range
power-law correlations as natural text at measurably different strength
\cite{kokol1999}: a variable must agree with its declaration and a state with its
machine however far apart they sit, so the whole corpus behaves less like prose and
more like a mechanism. Tighter local statistics with far deeper reach, exactly the
pairing this payload shows, is that signature. Whether the delivered stream is in
fact organised as something program-like is the question the independent
continuation noted in Section~\ref{sec:ledger} is built to answer, and it is a
measurement question rather than a speculation: an operation-like organisation makes
predictions about composition and execution order that prose does not.
...
\bibitem{hindle2012} Hindle, A., Barr, E.~T., Su, Z., Gabel, M., \& Devanbu, P.
(2012). On the naturalness of software. \emph{Proc. 34th International Conference on
Software Engineering (ICSE)}, 837--847.

\bibitem{kokol1999} Kokol, P., Podgorelec, V., Zorman, M., Kokol, T., \& Njivar, T.
(1999). Computer and natural language texts: a comparison based on long-range
correlations. \emph{J. Am. Soc. Inf. Sci.} \textbf{50}(14), 1295--1301.
...
\bibitem{kumarlinder2019} Kumar, P., \& Linder, E.~V. (2019). Use of fast radio
burst dispersion measures as distance measures. \emph{Phys. Rev. D} \textbf{100},
083533.
...
\bibitem{ocker2022} Ocker, S.~K., Cordes, J.~M., Chatterjee, S., et al. (2022). The
large dispersion and scattering of FRB~20190520B are dominated by the host galaxy.
\emph{Astrophys. J.} \textbf{931}, 87.
