...
\begin{abstract}
\noindent
Fast radio bursts are millisecond radio flashes from other galaxies, and a few
...
finds the working parts of one in the most densely sampled source,
FRB~20201124A, and finds the same multi-channel structure again at a second
source. Each
...
run one way in time. \textbf{2. The stripping.} Discarding the carrier and
separating the channels, the operation a receiver performs, isolates the
stream the transport layer was wrapping. \textbf{3. The language test.} The
statistical language tests of the animal-communication literature, applied to
that stream, pass on every axis, matching English or exceeding it on each:
context helps prediction 18 symbols back where English text run through the
identical pipeline, matched in length and alphabet, manages 2.3 symbols, and
six multi-burst signal motifs learned from early
sessions recur, recognisably, in later ones. The structure survives every
control built to explain it away, including a surrogate built to repeat
symbols exactly as often, and in runs exactly as long, as the real stream, and
it follows the source rather than the instrument: the
same statistical fingerprint tracks FRB~121102 through two different
telescopes, while different sources through one telescope stay distinct. Every
prediction testable on a third source, stated in advance, was confirmed.
...
subtle. One disambiguation for readers arriving from physics: \emph{entropy}
throughout this paper is Shannon's information measure, the average number of bits
needed to specify the next symbol given what is already known. It shares its
mathematics with thermodynamic entropy but measures uncertainty in a message, not
disorder in a medium; no thermodynamic quantity or claim appears anywhere in this
paper.
