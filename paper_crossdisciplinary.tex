...
\begin{abstract}
\noindent
Fast radio bursts are millisecond radio flashes whose sources are placed far
beyond our galaxy by the alterations their signals accumulate in transit;
nothing in this paper depends on that placement. A few
...
published, and report what each returns.
...
and the symbol sequence shows: a
repeating 32-symbol frame, a dominant carrier component with five weaker
sub-channels multiplexed beneath it, a
...
deliver. So exactly the machinery of step 1 is stripped away, blindly, nothing
...
undeciphered scripts and to search whale song for language, applied to
what remains, pass on every axis, matching English or exceeding it: context
...
structure survives every control built to explain it away, including a
...
individual flash, is the object.} Distance, by contrast, is an attribution
rather than a premise: sources are conventionally placed far beyond the galaxy
through models of the dispersion their signals accumulate, a quantity that
bounds distance only from above and is already known to overshoot
substantially where host galaxies have been measured
\cite{kumarlinder2019,ocker2022}, while the foreground
components of those models are still being revised against in-situ
measurement. Nothing here rests on the attribution; every statistic below is
